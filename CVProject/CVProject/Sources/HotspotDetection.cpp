...
void on_trackbarWindowSize(int, void*);
...
int myRoutine()
{
    ...
    Size winSize(myWinSize,myWinSize);
	...
	//cap.open("../2low.avi");
	cap.open("D:/ktmfbz/Documents/Dropbox/computer vision project/VIDEO_OK_TAGLIO.mov");
	...
	//createTrackbar( "Window Size", "Feature tracking", &trackbarWindowSize,
 //           50, on_trackbarWindowSize );
	...
	for(;;)
	{		
		if(frame_number%(int)(cap.get(CV_CAP_PROP_FPS)*0.5)==0 || frame_number==1)	//reinitialize if we are at the first frame or every 2 sec; frmae_rate->CV_CAP_PROP_FPS
		{ needToInit = true; }
		else 
		{ needToInit = false; }

		/*on_trackbarWindowSize(myWinSize, 0);
		Size winSize(myWinSize,myWinSize);*/
		Mat frame;
		...
		if(needToInit)
		{
			...
			goodFeaturesToTrack(gray, points[1], MAX_COUNT, 0.0005, 10, Mat(), 3, 0, 0.04);
...
void on_trackbarWindowSize(int, void*)
{
	myWinSize = 5 + (trackbarWindowSize * (50 - 5) / 50);
}
